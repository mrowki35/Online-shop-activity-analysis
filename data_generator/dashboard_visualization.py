...
# COMMAND ----------
status_df = df.withColumn("session_status", 
...
# COMMAND ----------
timeline_df = df.groupBy(window(col("window_start"), "1 hour").alias("time_window")) \
    .agg(
...
# COMMAND ----------
revenue_df = df.filter(col("session_revenue") > 0) \
...
# COMMAND ----------
conversion_df = df.select(
    lit("All Sessions").alias("step"), count("*").alias("count")
).union(
    df.filter(col("has_cart_activity") == 1).select(lit("Cart Activity").alias("step"), count("*").alias("count"))
).union(
    df.filter(col("is_purchased") == 1).select(lit("Purchased").alias("step"), count("*").alias("count"))
)

display(conversion_df)

# COMMAND ----------
top_users_df = df.groupBy("userId") \
    .agg(
